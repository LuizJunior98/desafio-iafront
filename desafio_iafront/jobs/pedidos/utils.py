...
def prepare(pedidos_joined: pd.DataFrame) -> pd.DataFrame:
...
def save_prepared(saida: str, visita_com_produto_e_conversao_df: pd.DataFrame):
    prepared = prepare(visita_com_produto_e_conversao_df)
...
def create_merge_visitas_produto_partitioned_by_hour(data: datetime, hour: int, pedidos: str, produtos_df: pd.DataFrame,
                                                     saida: str, visitas: str) -> str:
